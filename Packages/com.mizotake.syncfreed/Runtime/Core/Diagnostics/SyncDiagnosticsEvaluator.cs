...
namespace MizoTake.SyncFreeD.Core.Diagnostics
{
    public static class SyncDiagnosticsEvaluator
    {
        public static SyncDiagnosticsSnapshot Evaluate(in CameraSyncState state)
        {
            ...
            var observedLens = state.ObservedLens.FocalLengthMm != 0d ? state.ObservedLens : state.PredictedLens;
            var correctedLens = state.CorrectedLens.FocalLengthMm != 0d ? state.CorrectedLens : state.Lens;
