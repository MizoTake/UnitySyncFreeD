...
5. `com.unity.postprocessing` が project に入っている場合、sample の `BuiltinPostProcessDepthOfFieldTargetBehaviour` が `PostProcessLayer` / `PostProcessVolume` / `DepthOfField` を自動で組みます。sample 既定値は `Priority=100`, `Aperture=4`, `FocalLength=50`, `KernelSize=Medium` 相当です。`Focus +` / `Focus -` の操作に応じて被写界深度も変わります。
